...
import { _toMinutes, _toHour, _detectTime, _compareGreateThanTime } from './helpers';
...
class Timetable extends BaseScreen {
    ...
    _renderItemTime = ({ item, index }) => {
        return <View style={[styles.itemTime, { backgroundColor: index % 2 == 0 ? '#FFF' : buttonColor.bgInactive }]}>
    ...
    async _doCreateTimetable() {
        ...
        let timeOut = _timeoutCancelHandling(() => this.setState({ isHandling: false, isOpenModal: false }));
        let { schedule, daySelected, childSelected, timeCreateInDay } = this.state;
        ...
        schedule.start_at = moment(daySelected + ' ' + timeCreateInDay.startTime, 'YYYY-MM-DD HH:mm').format('DD-MM-YYYY HH:mm:ss');
        schedule.end_at = moment(daySelected + ' ' + timeCreateInDay.endTime, 'YYYY-MM-DD HH:mm').format('DD-MM-YYYY HH:mm:ss');
        ...
        if (response && response.status == 200 && response.data && response.data.schedule_info && !response.data.error) {
            ...
            this._resetForm();
            this._onUpdateEvent(this.isEditSchedule ? 1 : 0);
        ...
        if (childSelected.id == -1) {
            ...
            if (typeUpdate == 1) {
                ...
                new Promise(async (resolve) => {
                    ...
                    resolve(_arrEvents);
                }).then(async (val) => await this._getSchedule(true))
            } else await this._getSchedule(false);
        }
    }
    ...
    async _onUpdateTimetable() {
        ...
        setTimeout(async () => {
            if (childSelected.id != -1) {
                const { hourDatas } = this.state;
                let _arrEvents = await this._generateEvents(this.timetableConfig, hourDatas);
                this.setState({ _arrEvents }, async () => await this._getSchedule(true));
            } else {
                await this._getScheduleForAll();
            }
    ...
    async _getTimetableSetting() {
        ...
        if (timetableSetting) {
            ...
            this.setState({ hourDatas, dayDatas, _arrEvents, weekdays, lessonsLimited: timetableSetting.lessons });
        } else {
            const userToken = this.props.user.apiKey;
            ...
            if (settingRespon && settingRespon.status == 200 && settingRespon.data && settingRespon.data.start_time && !settingRespon.data.error) {
                ...
                this.setState({ hourDatas, dayDatas, _arrEvents, weekdays, lessonsLimited: settingRespon.data.lessons });
            } else {
    ...
    _getIndexActivity(_arrStarts, startAt) {
        let _temp = -1;;
        for (let i = 0; i < _arrStarts.length - 1; i++) {
            if (_arrStarts[i] && _toMinutes(startAt) >= _toMinutes(_arrStarts[i])) {
                _temp = i;
            } else break;
        }
        return _temp;
        ...
        arrE.forEach((e) => {
            ...
            if (!e.activity_id) {
                ...
                if (indexRowTime != -1) {
                    ...
                    _arr[indexRowTime].style = { borderTopWidth: 1, borderBottomWidth: 1 };
                    let _flag = 0;
                    for (let i = 1; i < _arr.length - 1; i++) {
                        let girdEndTime = _arr[indexRowTime + i].endTime;
                        if (_compareGreateThanTime(girdEndTime, _scheduleEnd)) {
                            if (_flag == 0) {
                                _arr[indexRowTime].style.borderBottomWidth = 0; // here
                            }
                            _flag += 1;
                            _arr[indexRowTime + i]._isContinute = true;
                            _arr[indexRowTime + i].parentData = _arr[indexRowTime];
                            _arr[indexRowTime + i].color = _arr[indexRowTime].color
                        } else {
                            if (_flag > 0) {
                                if (_arr[indexRowTime + i - 1].style) {
                                    _arr[indexRowTime + i - 1].style.borderBottomWidth = 1;
                                } else {
                                    _arr[indexRowTime + i - 1].style = {
                ...
                let indexRowTime = this._getIndexActivity(_arr.map((ea) => ea.startTime), moment(e.startTime).format('HH:mm'));
                if (indexRowTime != -1) {
                    if (_arr[indexRowTime].activitys) {
                        _arr[indexRowTime].activitys.push(e);
                    } else {
                        _arr[indexRowTime].activitys = [e];

                    }

                    // schedule.color = childSelected.color;
                    // if (_arrEvents[indexDayOfWeek - 1][indexRowTime].activitys) {
                    //     _arrEvents[indexDayOfWeek - 1][indexRowTime].activitys.push(schedule);
                    // } else {
                    //     _arrEvents[indexDayOfWeek - 1][indexRowTime].activitys = [schedule];
                    // }
    ...
    async _getScheduleForAll() {
        ...
        setTimeout(async () => {
            await _asyncForEach(_results, async (arrOfChilds, aindex) => {
                await _asyncForEach(arrOfChilds, async (eventOfChild, eindex) => {
                    if (eventOfChild.length > 0) {
                        eventOfChild = await this._preHandleEvent(eventOfChild, eindex);
                        // console.log("eventOfChild", eventOfChild)
            ...
            // console.log("here", _arrEvents)
            await _asyncForEach(_arrEvents, async (e, eindex) => {
                _flexEvent[eindex] = Array.isArray(e[0]) ? e.length : 1;
                if (e.length == 0) {
    ...
    async _getSchedule(isReloadActivity = true, childSelected = this.state.childSelected) {
                ...
                if (!schedule.activity_id) {
                    ...
                    if (indexRowTime != -1) {
                        if (childSelected.id == -1 || i < lessonsLimited) {
                            let _scheduleEnd = moment(schedule.end_at).format('HH:mm');
                            ...
                            _arrEvents[indexDayOfWeek - 1][indexRowTime].endTime = _scheduleEnd;
                            ...
                            _arrEvents[indexDayOfWeek - 1][indexRowTime].style = Object.assign({}, { borderTopWidth: 1, borderBottomWidth: 1 });
                            let _flag = 0;
                            for (let i = 1; i < _arrEvents[indexDayOfWeek - 1].length - 1; i++) {
                                let girdEndTime = _arrEvents[indexDayOfWeek - 1][indexRowTime + i].endTime;
                                if (girdEndTime == _scheduleEnd || _detectTime(girdEndTime).hour < _detectTime(_scheduleEnd).hour) {
                                    if (_flag == 0) {
                                        _arrEvents[indexDayOfWeek - 1][indexRowTime].style.borderBottomWidth = 0;
                                    }
                                    _flag += 1;
                                    _arrEvents[indexDayOfWeek - 1][indexRowTime + i]._isContinute = true;
                                    _arrEvents[indexDayOfWeek - 1][indexRowTime + i].parentData = _arrEvents[indexDayOfWeek - 1][indexRowTime];
                                    _arrEvents[indexDayOfWeek - 1][indexRowTime + i].color = _arrEvents[indexDayOfWeek - 1][indexRowTime].color
                                } else {
                                    if (_flag > 0) {
                                        if (_arrEvents[indexDayOfWeek - 1][indexRowTime + i - 1].style) {
                                            _arrEvents[indexDayOfWeek - 1][indexRowTime + i - 1].style = Object.assign({}, { borderTopWidth: 0, borderBottomWidth: 1 });
                                        } else {
                                            _arrEvents[indexDayOfWeek - 1][indexRowTime + i - 1].style = {
                                                borderTopWidth: 0, borderBottomWidth: 1
                                            };
                                        }
                                        _flag = 0;
                    ...
                    if (isReloadActivity) {
                        let indexRowTime = this._getIndexActivity(_arrEvents[indexDayOfWeek - 1].map((e) => e.startTime), moment(schedule.start_at).format('HH:mm'));
                        if (indexRowTime != -1) {
                            schedule.color = childSelected.color;
                            if (_arrEvents[indexDayOfWeek - 1][indexRowTime].activitys) {
                                _arrEvents[indexDayOfWeek - 1][indexRowTime].activitys.push(schedule);
    ...
    _handleScheduleAll() {
        return new Promise(async (resolve) => {
            ...
            let _from = moment().startOf('isoWeek').format('DD-MM-YYYY');
            let _to = moment().endOf('isoWeek').format('DD-MM-YYYY');
            await _asyncForEach(this.state.childs, async (c) => {
                let _eventOfChild = [[], [], [], [], [], [], []];
                let childId = c.id;
                let scheduleRespon = await sf.getServices('ScheduleService').getByRange(childId, _from, _to, userToken);
                if (scheduleRespon && scheduleRespon.data && !scheduleRespon.data.error) {
                    ...
                    for (let i = 0; i <= schedule.length - 1; i++) {
                        ...
                        // console.log("_eventOfChild", _eventOfChild)
                    }
                }
    ...
    _renderWeekdayOfChild({ item, index }) {
        if (this.state.childSelected.id == -1) {
            if (Array.isArray(item)) { // multiple child in a day
                return <View style={{ flex: 1 }}>
                    {item.map((_item) => {
                        if (_item.activitys && _item.activitys.length > 0) {
                            return <View style={{ flex: 1, height: scale(80) }} key={_item.id}>
                                {_item.activitys.map((_a, _index) => {
                                    return <TouchableWithoutFeedback key={_a.id}
                                    >
                                        <View style={{ flex: 1 }}>
                                            <View style={[
                                                styles.itemActivity,
                                                { borderTopColor: _a.color, borderBottomColor: _a.color },
                                                { borderRightColor: calendarColor.border },
                                                _a.title ? { backgroundColor: _hexToRgbA(_a.color) } : {}
                                            ]}>
                                                <StandardText small bold style={[styles.textEvent, _a.title ? { color: _a.color } : {}]}>{_shortText(_a.title, 16)}</StandardText>
                                            </View>
                                            {(_index + 1 < _item.activitys.length) && <View style={[styles.divider, { backgroundColor: _a.color }]} />}
                                        </View>
                            ...
                            >
                                <View style={[
                                    ...
                                    { borderRightColor: calendarColor.border },
                                    (_item.title || (_item._isContinute && _item.parentData)) ? { backgroundColor: _hexToRgbA(_item.color) } : { backgroundColor: _item.color },
                                    _item._selected ? this._renderBoderSelected() : {},
                                    _item.style ? _item.style : { borderTopWidth: 0, borderBottomWidth: 0 }
                                ]}>
            ...
            } else { // only 1 child in day
                if (item.activitys && item.activitys.length > 0) {
                    return <View style={{ flex: 1, height: scale(80) }} key={item.id}>
                        {item.activitys.map((_a, _index) => {
                            return <TouchableWithoutFeedback key={_a.id}
                            >
                                <View style={{ flex: 1 }}>
                                    <View style={[
                                    ...
                                    {(_index + 1 < item.activitys.length) && <View style={[styles.divider, { backgroundColor: _a.color }]} />}
                                </View>
                            </TouchableWithoutFeedback>
                        })}
                    </View>
                ...
                >
                    <View style={[
                        ...
                        { borderRightColor: calendarColor.border },
                        (item.title || (item._isContinute && item.parentData)) ? { backgroundColor: _hexToRgbA(item.color) } : { backgroundColor: item.color },
                        item._selected ? this._renderBoderSelected() : {},
                        item.style ? item.style : { borderTopWidth: 0, borderBottomWidth: 0 }
                    ]}>
            ...
            if (item.activitys && item.activitys.length > 0) {  // render event on timetable
                return <View style={{ flex: 1, height: scale(80) }}>
                    {item.activitys.map((_item, _index) => {
                        return <TouchableWithoutFeedback key={_item.id}
                        // onPress={this._onClickTimetable.bind(this, _item)}
                        >
                            <View style={{ flex: 1 }}>
                                <View style={[
                                    styles.itemActivity,
                                    { borderRightColor: calendarColor.border },
                                    _item.name ? { backgroundColor: _hexToRgbA(_item.color) } : { backgroundColor: _item.color || '#fff' }
                                ]}>
                                    <StandardText small bold style={[styles.textEvent, _item.name ? { color: _item.color } : {}]}>{_shortText(_item.name, 16)}</StandardText>
                                </View>
                                {(_index + 1 < item.activitys.length) && <View style={[styles.divider, { backgroundColor: _item.color }]} />}
                            </View>
                        </TouchableWithoutFeedback>
                    })}
                </View>
            }
            // render schedule for each child
            return <TouchableWithoutFeedback key={item.id}
                onPress={this._onClickTimetable.bind(this, item)}
            ...
            >
                <View style={[
                    styles.itemEvent,
                    // item.title ? {height: scale(160)} : {},
                    !item.activity_id ? { borderTopColor: item.color, borderBottomColor: item.color } : { borderTopWidth: 0, borderBottomWidth: 0 },
                    { borderRightColor: calendarColor.border },
                    (item.title || (item._isContinute && item.parentData)) ? { backgroundColor: _hexToRgbA(item.color) } : { backgroundColor: item.color || '#fff' },
                    item._selected ? this._renderBoderSelected() : {},
                    item.style ? item.style : { borderTopWidth: 0, borderBottomWidth: 0 }
                ]}>
                    {!!item.subject && <StandardText small bold style={[styles.textEventSubject, { color: item.color }]}>{_shortText(item.subject, 16)}</StandardText>}
    ...
    _renderBoderSelected() {
        return { backgroundColor: mainColor.main, opacity: .3 }
    }
    ...
    _onSelectTimeRange(timeSelected) {
        let { _arrEvents, childSelected } = this.state;
        let _dayOfWeeks = _arrEvents[timeSelected.dayOfWeek.id - 1]
        if (childSelected.id == -1 && Array.isArray(_arrEvents[timeSelected.dayOfWeek.id - 1][0])) {
            _dayOfWeeks = _arrEvents[timeSelected.dayOfWeek.id - 1][0];
        }
        let _indexTime = _dayOfWeeks.map((d) => d.startTime).indexOf(timeSelected.startTime);
        if (_indexTime != -1) {
            _dayOfWeeks[_indexTime]._selected = true;
        }
        this.setState({ _arrEvents })
    }

    _genTimeCreateInDay(_arr) {
        let _selects = _arr.filter((a) => a._selected);
        if (_selects.length > 0) {
            return {
                startTime: _selects[0].startTime,
                endTime: _selects[_selects.length - 1].endTime,
            }
        }
        return null;
    }
    ...
    async _onClickTimetable(timeSelected) {
        ...
        if (!timeSelected.activity_id) {
            ...
            // create schedule
            if (timeSelected.title == '' && !timeSelected.parentData) {
                let { _arrEvents, childSelected } = this.state;
                await this._checkPermission();
                if (this.state.childs.length <= 0) return noticeUtils.inform(noticeUtils.message.needCreateChild);
                if (this._isSelectTimeDay) {
                    let _dayOfWeeks = _arrEvents[timeSelected.dayOfWeek.id - 1]
                    if (childSelected.id == -1 && Array.isArray(_arrEvents[timeSelected.dayOfWeek.id - 1][0])) {
                        _dayOfWeeks = _arrEvents[timeSelected.dayOfWeek.id - 1][0];
                    }
                    if (this._isSelectTimeDay == timeSelected.dayOfWeek.id) {
                        let _indexTime = _dayOfWeeks.map((d) => d.startTime).indexOf(timeSelected.startTime);
                        if (_dayOfWeeks[_indexTime]._selected) {
                            if (this._scheduleUpdating) { // edit time of schedule
                                let timeCreateInDay = this._genTimeCreateInDay(_dayOfWeeks);
                                this.setState({
                                    isOpenModal: true,
                                    timeCreateInDay,
                                    isNoticeVisible: false
                                ...
                                _dayOfWeeks.forEach((a) => a._selected = false);
                                this._isSelectTimeDay = null;
                                this.setState({ _arrEvents })
                            } else {
                                // go create schedule
                                ...
                                this._isSelectTimeDay = null;
                                this.setState({ _arrEvents })
                                // end go create schedule
                            }
                        } else {
                            if (!_dayOfWeeks[_indexTime + 1]) {
                                if (_dayOfWeeks[_indexTime - 1]._selected) {
                                    this._onSelectTimeRange(timeSelected)
                                }
                            } else if (!_dayOfWeeks[_indexTime - 1]) {
                                if (_dayOfWeeks[_indexTime + 1]._selected) {
                                    this._onSelectTimeRange(timeSelected)
                                }
                            } else {
                                if (_dayOfWeeks[_indexTime - 1]._selected || _dayOfWeeks[_indexTime + 1]._selected) {
                                    this._onSelectTimeRange(timeSelected)
                                }
                            }
                        }
                    } else { // auto edit range time selected
                        let _oldDayOfWeeks = _arrEvents[this._isSelectTimeDay - 1];
                        if (childSelected.id == -1 && Array.isArray(_arrEvents[this._isSelectTimeDay - 1][0])) {
                            _oldDayOfWeeks = _arrEvents[this._isSelectTimeDay - 1][0];
                        }
                        _oldDayOfWeeks.forEach((a) => a._selected = false);
                        this._onSelectTimeRange(timeSelected);
                        this._isSelectTimeDay = timeSelected.dayOfWeek.id;
                    }
                } else { // start select range time
                    this._onSelectTimeRange(timeSelected);
                    this._isSelectTimeDay = timeSelected.dayOfWeek.id;
                }
            } else {
                // detail schedule
                if (timeSelected.parentData) timeSelected = timeSelected.parentData;
                let timeCreateInDay = {
                    startTime: timeSelected.startTime,
                    endTime: timeSelected.endTime,
                }
                this.setState({
                    timeCreateInDay,
                    schedule: {
    ...
    async _onCopySchedule(timeSelected) {
        ...
        await this._checkPermission();
        if (timeSelected.title) { // modal copy show
            if (!timeSelected.activity_id) {
                this.setState({
            ...
            if (this.state.scheduleCopy) { // modal paste show
                this.setState({
    ...
    async _onChildSelected(childSelected) {
        if (childSelected.id != -1) {
            ...
            }, () => this._getSchedule(true, childSelected));
        } else {
    ...
    async _doPasteSchedule(schedulePaste) {
        ...
        let _start = moment(`${moment().format('YYYY-MM-DD')} ${schedulePaste.startTime}`, 'YYYY-MM-DD HH:mm');
        let _end = moment(`${moment().format('YYYY-MM-DD')} ${schedulePaste.endTime}`, 'YYYY-MM-DD HH:mm');
        let _diffHour = _end.diff(_start, 'minute');
        schedule.start_at = moment(daySelected + ' ' + timeSelected.startTime, 'YYYY-MM-DD HH:mm').format('DD-MM-YYYY HH:mm:ss')
        schedule.end_at = moment(daySelected + ' ' + timeSelected.startTime, 'YYYY-MM-DD HH:mm').add(_diffHour, 'minute').format('DD-MM-YYYY HH:mm:ss')
    ...
    _doCancelEditTime() {
        let { timeSelected, _arrEvents } = this.state;
        let _dayOfWeeks = _arrEvents[timeSelected.dayOfWeek.id - 1]
        _dayOfWeeks.forEach((a) => a._selected = false);
        this.setState({ _arrEvents, isNoticeVisible: false, isOpenModal: true });
        this._isSelectTimeDay = null;
        this._scheduleUpdating = false;
    }
    ...
    render() {
        const { isHandling, schedule, hourDatas, timeSelected, timeCreateInDay } = this.state;
        if (isHandling) {
        ...
        return (
            <View style={{ flex: 1 }}>
                <ViewShot ref="viewShot" options={{ format: "png", quality: 1, result: 'base64' }} style={{ flex: 1 }}>
                    ...
                    {this.state.isNoticeVisible && <Animatable.View animation="slideInUp" style={styles.viewNotice}>
                        <StandardText>화면에서 시간을 선택하십시오</StandardText>
                        <SimpleButton
                            onPress={this._doCancelEditTime.bind(this)}
                            propComponent={{
                                underlayColor: buttonColor.underlay
                            }}
                            style={styles.btnCancelEdit}
                            textStyle={styles.textActiveStyle}>취소</SimpleButton>
                    </Animatable.View>}
                </ViewShot>
                <Modal
                        ...
                        >
                            <View style={modalStyles.containderModal}>
                                ...
                                <View style={modalStyles.contentModal}>
                                    <TouchableWithoutFeedback onPress={this._goChangeTime.bind(this)}>
                                        <View style={modalStyles.viewTimePicked}>
                                            <StandardText bold>
                                                {timeSelected.dayOfWeek && timeSelected.dayOfWeek.title} {timeCreateInDay.startTime}~{timeCreateInDay.endTime}
                                            </StandardText>
                                        </View>
                                    </TouchableWithoutFeedback>
                                    <AdvanceInput
                ...
                />}
                {this.state.isSyncing && <LoadingScreen title="동기화 됨" />}
            </View>
        )
    }
